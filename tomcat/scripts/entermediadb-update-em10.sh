...
if [ -z "$BUILD_NUMBER" ]; then
  curl -XGET -o /tmp/ROOT.war http://dev.entermediasoftware.com/jenkins/view/EM10/job/em10_demoall/lastSuccessfulBuild/artifact/deploy/ROOT.war > /dev/null
  status=$?
  if [ $status -ne 0 ]; then
  ...
  curl -XGET -o /tmp/ROOT.war http://dev.entermediasoftware.com/jenkins/view/EM10/job/em10_demoall/"$BUILD_NUMBER"/artifact/deploy/ROOT.war > /dev/null
  status=$?
  if [ $status -ne 0 ]; then
