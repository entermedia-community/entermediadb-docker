...
#####################################
#
# Launch EnterMediadb 11.x instance
#
#####################################
...
# For dev, will be updated to entermedia11
...
echo 'sudo docker exec -it -u 0 '$INSTANCE' entermediadb-update-em10.sh $1 $2' > ${SCRIPTROOT}/update-em11.sh
...
docker run -t -d \
	...
	entermediadb/$DOCKERIMAGE:$BRANCH \
